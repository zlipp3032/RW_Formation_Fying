function [agent,leader_agent,inter_agent] = AnalyzeThreeAgent(data1,data2,data3,target_vectors)
...
%     index1 = find(A1.v1_flightSequence > 0);
    index1 = data1.index;
    agent(1).time = A1.RelTime(index1);
    
    % Set the index_form parameter
    agent(1).flight_sequence = A1.v1_flightSequence(index1);
    ...
    agent(1).pos_1 = [A1.v1_xPos(index1),A1.v1_yPos(index1),A1.v1_zPos(index1)];
    agent(1).vel_1 = [A1.v1_xVel(index1),A1.v1_yVel(index1),A1.v1_zVel(index1)];
    
    % Smooth the agent velocity and estimate the actual acceleration using
    % MATLAB 'sgolayfilt' function
    agent(1).vel_sgo = [sgolayfilt(A1.v1_xVel,2,11),sgolayfilt(A1.v1_yVel,2,11), sgolayfilt(A1.v1_zVel,2,11)];
    agent(1).acc_sgo = [[0,0,0];diff(agent(1).vel_sgo)./[diff(A1.RelTime) diff(A1.RelTime) diff(A1.RelTime)]];
    
    % Get the acceleration input from the flights
    agent(1).ctrl = [A1.v1_ux(index1), A1.v1_uy(index1), A1.v1_uz(index1)];
    
    % Set the throttle parameters
    agent(1).throttle = A1.v1_tPWM_des;
    
    % Set the battery parameters
    agent(1).battery = A1.v1_battVolt;
    
    % Get the Relative to Target Values
    [agent(1).R2T,~] = computeR2T(index1,A1,target_vectors);
    
    % Get the leader-agent errors
    leader_agent(1).pos_error_1 = agent(1).pos_1 - [A1.v1_leadXPos(index1),A1.v1_leadYPos(index1),A1.v1_leadZPos(index1)];
    leader_agent(1).vel_error_1 = agent(1).vel_1 - [A1.v1_leadXVel(index1),A1.v1_leadYVel(index1),A1.v1_leadZVel(index1)];
